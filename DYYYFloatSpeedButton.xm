...
- (void)handlePan:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        CGFloat minY = safeAreaTop + self.frame.size.height / 2;
        CGFloat maxY = self.superview.frame.size.height - safeAreaBottom - self.frame.size.height / 2;
        
        // 边界检查（增加安全区限制）
        newCenter.x = MAX(self.frame.size.width / 2, MIN(newCenter.x, self.superview.frame.size.width - self.frame.size.width / 2));
        newCenter.y = MAX(minY, MIN(newCenter.y, maxY)); // 修改Y轴边界检查
